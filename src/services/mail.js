...
// -------------------- BREVOR SERVICE -----------------
const FROM_NAME = 'Toothhive';
const FROM_EMAIL = process.env.EMAIL_APP;
...
const sendMail = async ({ to, subject, html, attachments = [] }) => {
	...
	return client.transactionalEmails.sendTransacEmail(body);
};
...
const billInfo = async (data) => {
	...
	await sendMail({
        ...
        <p>Nếu bạn nghĩ đây là sự nhầm lẫn, vui lòng liên hệ với chúng tôi để được hỗ trợ.</p>
        `,
		attachments: [{ filename: data.filename, content: fileBuffer }],
	});
};
